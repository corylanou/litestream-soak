...
func ensureEvidenceJournal(db *sql.DB) error {
 ...
 INSERT OR IGNORE INTO evidence_verifications SELECT %[1]s, COALESCE(NULLIF(json_extract(v.run_identity_json,'$.source'),''),w.source,''), w.region, w.profile_name, w.last_runtime_json, 0 FROM verifications v LEFT JOIN workers w ON w.id=v.worker_id;
...
func (d *DB) ListEvidenceVerifications(source string) ([]EvidenceVerification, error) {
	rows, err := d.query(`SELECT `+evidenceVerificationColumns+`, COALESCE(evidence_region,''), COALESCE(evidence_profile,''), COALESCE(evidence_runtime,''), history_complete FROM evidence_verifications WHERE (evidence_source=? OR evidence_source='') ORDER BY COALESCE(completed_at, started_at), id`, source)
	if err != nil {
		return nil, err
	}
...
func (d *DB) ListEvidenceEvents(source string) ([]Event, error) {
	rows, err := d.query(`SELECT id, COALESCE(worker_id,''), event_type, message, COALESCE(details,''), created_at FROM evidence_events WHERE (evidence_source=? OR evidence_source='') ORDER BY created_at,id`, source)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
func (d *DB) ListRuntimeEvidence(source string, deploymentID int) ([]RuntimeEvidence, error) {
	rows, err := d.query(`SELECT id,identity_json,runtime_json,attributed,received_at,kind FROM evidence_runtime WHERE (source=? OR source='') AND deployment_id IN (?,0) ORDER BY id`, source, deploymentID)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
