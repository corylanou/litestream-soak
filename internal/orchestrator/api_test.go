...
func TestHandleVerificationPendingReportIsNeutral(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			db := openTestDB(t)
			workerID := "worker-pending-" + tt.name
			createTestWorker(t, db, model.Worker{
				ID:            workerID,
				Name:          workerID,
				Status:        model.WorkerRunning,
			...
			if verifications[0].Status != "pending" {
				t.Fatalf("verification Status = %q, want pending", verifications[0].Status)
			}
			if verifications[0].Passed {
				t.Fatalf("verification Passed = true, want false")
			}
			...
			if !foundPending {
				t.Fatalf("verification_pending event not recorded; got: %+v", events)
			}
		})
	}
}
