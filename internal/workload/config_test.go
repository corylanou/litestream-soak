...
func TestParseConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "valid JSON",
			input: `{"write_rate":100,"load_mode":"synthetic","monitor_interval":"1s","verify_sync_degraded_after":"1m","verify_sync_timeout":"3m","disk_full_no_progress_window":"2m","disk_full_recovery_reserve_bytes":314572800,"disk_full_recovery_timeout":"5m","s3_fault_proxy_require_observed_source_range_get":true}`,
			want: Config{
				WriteRate:                                 100,
				LoadMode:                                  "synthetic",
				MonitorInterval:                           "1s",
